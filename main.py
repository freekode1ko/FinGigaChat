...
def main():
    ...
    for table in all_tables:
        ...
        # EXCENGE BLOCK
            ...
            if set(['Exchange', 'Last', 'Time']).issubset(table[3].columns):
            ...
            pass  # TODO: parce HTML for currency
    ...
    big_table = pd.concat([big_table, metals_coal_kot_table], ignore_index=True)
    big_table.to_excel(metal_writer, sheet_name='Металы')

    exchange_writer = pd.ExcelWriter('sources/tables/exc.xlsx')
    pd.DataFrame(exchenge_kot, columns=['Валюта', 'Курс']).to_excel(exchange_writer, sheet_name='Курсы валют')
