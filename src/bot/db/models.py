...
class UserMeeting(Base):
    ...
    user_id = Column(ForeignKey('user.user_id', onupdate='CASCADE', ondelete='CASCADE'), nullable=False)
...
class CallReports(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(ForeignKey('user.user_id', ondelete='CASCADE', onupdate='CASCADE'),
