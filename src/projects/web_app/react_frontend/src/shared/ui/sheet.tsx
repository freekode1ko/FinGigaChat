...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-white text-dark-blue border-secondary-100 dark:border-secondary-600 dark:bg-dark-blue dark:text-white p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      side: {
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn('text-lg font-semibold text-text-color', className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn('text-sm text-hint-color', className)}
    {...props}
  />
))
