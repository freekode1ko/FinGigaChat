...
async def set_user_subscriptions(message: types.Message, state: FSMContext) -> None:
    ...
    keyboard = InlineKeyboardBuilder()
    continue_button = types.InlineKeyboardButton(text='Завершить', callback_data='end_write_subs')
    ...
    if len(subscriptions) < len(user_request):
        ...
        near_to_list_of_unknown = fuzzy_searcher.find_nearest_to_subjects_list(list_of_unknown)
    ...
    if subscriptions:
        ...
        keyboard = InlineKeyboardBuilder([[continue_button, ]]).as_markup()
